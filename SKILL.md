...
description: >-
  學習共同體公開課的逐字稿證據品質技能：把課堂錄影轉成可信的引用證據，再據以產出課例分析、16:9 簡報與議課現場播放的影片片段。核心是字幕審稿校正——音訊增益、Groq Whisper 帶詞彙表轉譯、清除幻覺字幕、對照教材課本原文逐字校正、重聽確認可疑語句，避免整份分析建立在辨識錯誤上。當使用者說「校正字幕」「字幕對不上課文」「逐字稿有錯字」「重聽這段」「學生到底說了什麼」「燒字幕」「議課要播的影片片段」「把字幕燒進影片」「課堂錄音太小聲」「聽不清楚學生說話」，或要對公開課影片做需要精確引用的課例分析時使用。若重點在學習共同體的哲學視角詮釋與抹茶綠繪本插圖簡報美學，改用 slc-skill。
---
